...
- **Snap environment**: `$SNAP_COMMON/solutions/activeConfiguration/he-ctrlx-app-commsw/outputs.json`
- **Development environment**: `~/My Documents/My ctrlX/he-ctrlx-app-commsw/outputs.json`
...
```json
{
  ...
  "MinStorageAvailable": 50,
  "MaxItemsInQueue": 200,
  "StoragePath": "/media/mmcblk1p1/Buffer/Stahlwille",
  "_comment": "StoragePath examples - external storage requires removable-media plug:",
  "_examples": {
    "external_sd_card": "/media/mmcblk1p1/Buffer/Stahlwille",
...
- **Snap environment**: `$SNAP_COMMON/solutions/activeConfiguration/he-ctrlx-app-commsw/OutputBuffer/`
- **Development environment**: `~/My Documents/My ctrlX/he-ctrlx-app-commsw/OutputBuffer/`
...
### Curve Data Encoding (Stahlwille)
...
**Path**: `he/commsw/app/tool1/lastResult`
...
| `total time` | string | Total tightening duration (seconds, may be `"0.000000"` for Stahlwille) |
| `tool serial` | string | Tool serial number (duplicates `channel` for compatibility) |

#### Tightening Steps Array

Each step represents a phase of the tightening process (Stahlwille typically has one step):
...
5. **Verify transmission** by monitoring `he/commsw/buffer/status` node or checking gateway logs
...
5. Check `he/commsw/buffer/status` node:
...
**Checks**:
1. Check `available_disk_space_mb` in `he/commsw/buffer/status` node
2. Compare against `MinStorageAvailable` setting (default 50 MB)
3. Check storage path exists and is writable:
   - Snap: `$SNAP_COMMON/solutions/activeConfiguration/he-ctrlx-app-commsw/OutputBuffer/`
...
- Connect `removable-media` snap plug: `snap connect he-ctrlx-app-commsw:removable-media`
...
**Checks**:
1. Check `queued_items` in `he/commsw/buffer/status` node
